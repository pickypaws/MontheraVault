...
async function switchToMonad() {
  try {
    ...
    if (switchError.code === 4902) {
      ...
      throw new Error("Gagal switch ke jaringan Monad.");
    }
  }
}
...
stakeInput.style.border = "1px solid #ccc"; // border default abu
...
connectIcon.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg"
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7" />
      ...
      console.warn("Auto-connect gagal:", err);
    }
  } else {
  if (loadingInterval) clearInterval(loadingInterval);
...
async function refreshUI() {
  try {
    await loadInfo(); // ambil ulang data saldo, staking, reward
    enableActionButtons(); // aktifkan tombol setelah info terupdate
  } catch (error) {
...
function resetUI() {
  ...
  // Ganti disabled dengan efek warning
  ...
  if (!isConnected) {
    ...
    try {
      ...
      if (typeof window.ethereum !== "undefined") {
        ...
        alert("Wallet provider tidak ditemukan. Gunakan MetaMask.");
        return;
      }

      const message = "Sign the message to proceed with Monthera Vault staking";
      ...
      console.error("Gagal konek/sign/switch:", err);
    ...
    try {
      ...
      showNotification("Sign-out successful", "error");
      document.getElementById("stakeAmount").value = "";
      resetStakeInputStyle();
      resetUI();
...
async function stake() {
  ...
  if (!inputRaw || isNaN(inputRaw) || parseFloat(inputRaw) <= 0) {
    alert("Jumlah stake tidak valid!");
    return;
  }
...
document.getElementById("redeemBtn").addEventListener("click", () => {
  ...
  if (!userAddress) {
    showWalletWarning("Please connect your wallet first");
  ...
  // Ambil input dan reset gaya/input
  const input = document.getElementById("redeemVaultInput");
  input.value = "";
  input.style.border = "1px solid #ccc";
  input.style.color = "";

  // ✅ Ambil dan set ulang nilai awal dari wallet user
  ...
  // ✅ Panggil preview awal biar sinkron
  updateRedeemPreview();
...
redeemInputEl.addEventListener("input", () => {
  ...
  // 🛑 Cegah getar jika tidak sedang diketik manual (misalnya di-trigger oleh fillPercentage)
...
document.getElementById("nextRedeemBtn").addEventListener("click", async () => {
  ...
  if (!rawInput || isNaN(inputValue) || inputValue <= 0) {
    ...
    return; // ⛔ Tidak ada showNotification di sini
  }

  // ❌ Melebihi share → Tampilkan notif khusus
  if (inputValue > vaultBalance) {
    triggerInputError(inputEl, true);
    updateRedeemPreview(true);
    showNotification("Insufficient balance", "error"); // ✅ Hanya di sini notif
    return;
  }
...
document.getElementById("confirmRedeemFinalBtn").addEventListener("click", async () => {
  ...
  const intervalId = setInterval(() => {
    ...
    confirmBtn.innerHTML = `${baseText}${dots}${spaces}`;
  }, 400);

  try {
    await new Promise((resolve) => setTimeout(resolve, 2000)); // delay animasi
    ...
    confirmBtn.innerHTML = `<i class="fas fa-check-circle text-green-500 text-3x3 fade-icon"></i>`;

    // Tunggu 2 detik agar ikon ceklis tampil dulu
    setTimeout(async () => {
      confirmBtn.innerHTML = originalText;
      confirmBtn.disabled = false;
      document.getElementById("redeemStep2").style.display = "none";

      // Baru jalankan notifikasi dan animasi setelah modal dihilangkan
    ...
    clearInterval(intervalId);
    confirmBtn.innerHTML = `<i class="fas fa-times-circle text-red-500 text-3x3 fade-icon"></i>`;
    console.error("Redeem failed:", err);
    showNotification("Redeem failed", "error");

    // Modal tetap terbuka saat gagal, hanya reset tombol
    setTimeout(() => {
      confirmBtn.innerHTML = originalText;
      confirmBtn.disabled = false;
    }, 2000);
...
async function claim() {
  ...
  const intervalId = setInterval(() => {
    ...
    const dots = ".".repeat(dotCount);
    const spaces = "&nbsp;".repeat(maxDots - dotCount);
    claimBtn.innerHTML = `${baseText}${dots}${spaces}`;
  }, 400);

  try {
    await new Promise(resolve => setTimeout(resolve, 2000)); // biar animasi muncul

    // ✅ Ambil reward SEBELUM transaksi, bukan dari elemen HTML
    ...
    document.getElementById("stakeAmount").value = "";
    clearInterval(intervalId);
    claimBtn.innerHTML = `<i class="fas fa-check-circle text-green-500 text-3x3 fade-icon"></i>`;

    // ✅ Gunakan angka yang benar-benar diklaim untuk animasi
    ...
    clearInterval(intervalId);
    claimBtn.innerHTML = `<i class="fas fa-times-circle text-red-500 text-3x3 fade-icon"></i>`;
  } finally {
    setTimeout(() => {
...
function showFlyUpBalance(amount) {
  ...
  // Setelah 5 detik, mulai proses hide (fade out)
  setTimeout(() => {
    flyup.classList.remove("show");

    // Setelah animasi fade out selesai (1.5s), hapus teks
    setTimeout(() => {
      flyup.textContent = "";
    }, 1500); // sesuai durasi transition di CSS
  }, 1500);
}
...
function animateDots() {
  ...
  clearInterval(button.dotInterval); // prevent duplicate intervals
...
function showWalletWarning(message) {
  const popup = document.getElementById("walletWarningPopup");
...
async function updateRewardRateUI() {
  try {
    const rewardRate = await vaultContractPublic.rewardRate(); // ambil dari contract (BigNumber)
    
    // Format dari wei ke MTHR per detik
    ...
    console.error("Gagal ambil rewardRate:", err);
...
function fillDepositPercentage(pct) {
  ...
  // ✅ Hitung persentase dan isi input
  ...
  if (input) {
    ...
    // ✅ Reset border merah jika sebelumnya error
    input.style.border = "1px solid #ccc";
    input.style.color = "";
...
function fillPercentage(pct) {
  ...
  // ✅ Reset border merah, warna, dan timeout error jika ada
  input.style.border = "1px solid #ccc";
  input.style.color = "";
  if (window.redeemInputErrorTimeout) {
    clearTimeout(window.redeemInputErrorTimeout);
  ...
  // ✅ Trigger input event agar update preview & validasi
...
document.getElementById("stakeBtn").addEventListener("click", async () => {
  ...
  if (!inputRaw || isNaN(inputValue) || inputValue <= 0) {
    // Hapus timeout lama jika masih aktif
    ...
    // Border merah dan animasi getar
    stakeInput.style.border = "2px solid red";
    stakeInput.style.color = "red";
    stakeInput.style.transition = "transform 0.1s";
    stakeInput.style.transform = "translateX(-4px)";
  ...
  try {
    ...
    console.error("Gagal hitung preview deposit:", err);
    alert("Gagal estimasi deposit: " + err.message);
  }
});
...
document.getElementById("confirmDepositBtn").addEventListener("click", async () => {
  ...
  try {
    await new Promise(resolve => setTimeout(resolve, 2000)); // animasi awal 2 detik
    ...
    // TUNDA semua aksi agar ikon ceklis muncul dulu
    setTimeout(async () => {
      confirmBtn.innerHTML = originalText;
      confirmBtn.disabled = false;
      hideModal("previewDepositModal");

      // Baru tampilkan notifikasi & efek animasi setelah modal ditutup
    ...
    // Reset tombol setelah delay, modal tetap terbuka
    setTimeout(() => {
      confirmBtn.innerHTML = originalText;
      confirmBtn.disabled = false;
    }, 2000);
...
async function loopReward() {
  ...
  try {
    ...
    // Delay manual yang dijamin jalan di WebView
    setTimeout(() => {
      requestAnimationFrame(loopReward); // lebih stabil dari timeout langsung
    }, 2100);
  }
}
...
function triggerInputError(inputEl, force = false) {
  ...
  // 🚨 Jika force = true, langsung jalankan efek
  if (force || isInvalid) {
...
function updateRedeemPreview(force = false) {
  ...
  // 🔴 Melebihi shares → anggap all-in dengan error visual
  ...
  // ❌ Invalid → TAMPILKAN nilai valid terakhir
  else {
